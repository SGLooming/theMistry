...
{
    return EProvider::where("id", $provider_id)->value('name');
}
...
function getProviderNames($providers)
{
    ...
    foreach ($providers as $provider) {
        $res[] = '<a href="' . route('eProviders.edit', $provider) . '" class="text-bold text-dark">{' . provider($provider) . '}</a>';
    }
    return implode(', ', $res);
}
function getAdharrCardName($column, $attributeName)
{
    if (isset($column)) {
        if ($column[$attributeName]) {
            return "<span class='badge badge-success'>" . trans('lang.yes') . "</span>";
...
{
    return EService::where('id', $service_id)->value('name');
}
function getCategoryName($service_id)
{
    $category_id =  DB::table('e_service_categories')->where('e_service_id', $service_id)->value('category_id');
...
function getServiceMedia($service_id)
{
    $mediaModel = Media::where('model_type', 'App\Models\EService')
        ->where('model_id', $service_id)
        ->where('collection_name', 'image')
        ->first();
    if ($mediaModel) {
        return $mediaModel->getFullUrl();
    } else {
        return "";
    }
    // dd( $mediaModel->getFullUrl());
}
