...
class EServiceController extends Controller
{
    ...
    public function store(CreateEServiceRequest $request)
    {
        ...
        try {
            ...
            if (isset($request->e_provider_id)) {
                foreach ($request->e_provider_id as $provider_id) {
                    $provider = new ProviderService();
                    $provider->provider_id = $provider_id;
                    $provider->service_id = $eService->id;
                    $provider->save();
                }
            }
    ...
    public function edit(int $id)
    {
        ...
        $eProvider = $this->eProviderRepository->getByCriteria(new EProvidersOfUserCriteria(auth()->id()))->pluck('name', 'id');
        $provider_services = ProviderService::where('service_id', $id)->pluck('provider_id')->toArray();
        return view('e_services.edit', compact('provider_services'))->with('eService', $eService)->with("customFields", isset($html) ? $html : false)->with("category", $category)->with("categoriesSelected", $categoriesSelected)->with("eProvider", $eProvider);
    }
    ...
    public function update(int $id, UpdateEServiceRequest $request)
    {
        ...
        try {
            ...
            ProviderService::where('service_id', $id)->delete();
            if (isset($request->e_provider_id)) {
                foreach ($request->e_provider_id as $provider_id) {
                    $provider = new ProviderService();
                    $provider->provider_id = $provider_id;
                    $provider->service_id = $id;
                    $provider->save();
                }
            }
            if (isset($input['image']) && $input['image'] && is_array($input['image'])) {
